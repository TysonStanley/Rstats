...
We use this code to get the data ready for the rest of the chapter. We use this code for the data for each chapter hereafter.
```{r, message=FALSE}
## Run but not shown
...
```{r}
fit3 = lm(famsize ~ race + dmdmartl, data=df)
summary(fit3)
```
...
fit_anova <- aov(famsize ~ minority*dmdmartl, df)
anova(fit_anova)
```
Notice two things: First, the interaction is significant (p = .003). This is important since we are going to try to interpret this interaction. Second, by including `minority*dmdmartl` we get both the main effects and the interaction. This is very important for interpretation purposes so you can thank `R` for making it a bit more easy on you. 
...
```{r}
fit_reg <- lm(famsize ~ minority*dmdmartl, df)
summary(fit_reg)
```
