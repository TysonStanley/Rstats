...
df <- dem_df %>%
  ...
  mutate(race = factor(ridreth1, 
                       labels=c("MexicanAmerican", "OtherHispanic", 
                                "White", "Black", "Other"))) %>%
  mutate(famsize = as.numeric(dmdfmsiz))
```
...
1. IF depression $\geq 16$ then dep2 is 1, 
2. IF dpression $< 16$, then dep2 is 0, 
3. ELSE dep2 is NA.
...
Similar to that above, negative binomial is for zero-inflated counts. It makes slightly different assumptions than the hurdle.
