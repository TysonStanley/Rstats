...
Let's start with a simple walk-through of data types, functions, and objects.
...
In `R` there are three main data types (variable types) that you'll work with in research:
...
For now, know that each function has a name (the function name of `c()` is "c"), arguments, and output of some sort. Arguments are the information that you provide the function (e.g. we gave `c()` a bunch of numbers; we gave `factor()` two arguments---the variable and the labels for the variable's levels). Output from a function varies to a massive degree but, in general, the output is what you are using the function for (e.g., for `c()` we wanted to create a vector---a variable---of data). At any point, by typing:
...
Below we show you several functions for both importing and saving data. Note that each have a name, arguments, and output of each. 
