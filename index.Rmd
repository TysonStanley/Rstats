...
Science. It is built on replication and construction. Once an idea is established and well replicated, the next steps are to build on to it. We try to build from a foundation established by others. It should also be how we analyze our data, not only in methodology but also in software. Lucky for us, `R` is how we do just that.

This book is for beginners in `R`; especially those in health, behavioral and social sciences. `R` is wonderfully unique. It is an open source project and improvements---known as packages---are being made by smart individuals across the globe.  
