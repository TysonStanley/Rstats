...
Not surprisingly, when we compared the ANOVA and the simple linear model, they are *exactly* the same. For a more interesting comparison, lets run a new model and then make a comparison.
...
Notice that the variable is associated with the outcome according to the t-test seen in the summary. So we would expect that `fit3` is better than `fit2` at explaining the outcome.
...
Both provide simple functions to compare multiple models.
